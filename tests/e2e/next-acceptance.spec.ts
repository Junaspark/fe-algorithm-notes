...
for (const viewport of [{ name: 'desktop', width: 1440, height: 900 }, { name: 'mobile', width: 390, height: 844 }]) {
  test(`${viewport.name} real practice page runs Worker and persists draft/submission handlers`, async ({ page, context, browser }) => {
    ...
    const editor = page.getByRole('textbox', { name: 'Editor content' }); await editor.focus(); await page.keyboard.press('ControlOrMeta+A'); await page.keyboard.insertText('const uniqueArray = arr => [...new Set(arr)]')
...
test('real Worker times out then recovers and completed plan creates one job of each kind', async ({ page, context, request }) => {
  ...
  await editor.focus(); await page.keyboard.press('ControlOrMeta+A'); await page.keyboard.insertText('const uniqueArray = () => { while(true){} }'); await page.getByRole('button', { name: '运行测试' }).click(); await expect(page.locator('[data-status="timeout"]')).toBeVisible({ timeout: 5000 })
  await editor.focus(); await page.keyboard.press('ControlOrMeta+A'); await page.keyboard.insertText('const uniqueArray = arr => [...new Set(arr)]'); await page.getByRole('button', { name: '提交解答' }).click(); await expect(page.getByText('已通过全部测试')).toBeVisible()
