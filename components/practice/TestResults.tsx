...
export default function TestResults({ result }: { result: RunResult | null }) {
  ...
  return <div className="test-results"><strong>{passed} / {result.tests.length} 通过</strong><ul>{result.tests.map(test => <li key={test.name} data-status={test.status}><span>{test.status === 'passed' ? '✓' : '×'}</span>{test.name}<small>{test.durationMs}ms</small></li>)}</ul></div>
}
